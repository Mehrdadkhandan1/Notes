...
const Todo = ({ data }) => {
  ...
  const editTodo = () => {
    ...
    axios
      .put(`/api/updateTodo/${_id}`, {
        ...data,
        title: valueInputEdit,
      })
      .then((resp) => {
        if (resp.status === 200) {
          //   set state
          dispatch({ type: "UPDATE_TODO", data: resp.data.data });
          //   close input
      ...
      <div className={style.todoBox}>
        ...
        >
          {isCompleted && (
            ...
            >
              <path
                d="M5.59509 14L0 7.36383L1.39877 5.70478L5.59509 10.6819L14.6012 0L16 1.65904L5.59509 14Z"
                fill="#BEBEBE"
              />
            </svg>
          )}
        ...
        {/* نمایش دکمه لینک */}
        {noteId ? (
          <Link to={`/shownote/${noteId}`} className={style.linkBtn}>
            <svg
              viewBox="0 0 16 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
      ...
      <div className={`${style.actionBtn} ${hasOpen ? style.hsaOpen : ""}`}>
        ...
        {showInput ? (
          <button
            ...
            className={style.btnSubmitedEdit}
          >
            {" "}
            submit{" "}
          </button>
          ...
          >
            ...
            >
              <path
                d="M11.2728 2.98294L13.0171 4.72637M12.3945 1.44704L7.67807 6.16344C7.43438 6.40679 7.26818 6.71684 7.20042 7.05451L6.76477 9.23524L8.9455 8.79876C9.28315 8.73123 9.5928 8.5657 9.83657 8.32193L14.553 3.60553C14.6947 3.4638 14.8071 3.29555 14.8838 3.11037C14.9605 2.92519 15 2.72672 15 2.52628C15 2.32585 14.9605 2.12738 14.8838 1.9422C14.8071 1.75702 14.6947 1.58877 14.553 1.44704C14.4112 1.30531 14.243 1.19288 14.0578 1.11618C13.8726 1.03948 13.6742 1 13.4737 1C13.2733 1 13.0748 1.03948 12.8896 1.11618C12.7045 1.19288 12.5362 1.30531 12.3945 1.44704Z"
                stroke="#BEBEBE"
                strokeWidth="1.5"
                strokeLinecap="round"
